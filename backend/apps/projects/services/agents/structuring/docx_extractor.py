...
class DocxExtractor:
    ...
    async def get_url(self):
        ...
        if project and hasattr(project, 'files'):
            # 使用 sync_to_async 包装 files.first() 调用
            first_file = await sync_to_async(project.files.first)()
            if first_file:
                return await sync_to_async(first_file.get_presigned_url)()
        return None
