...
import { DocxExtractionFastApi } from '@/components/projects/TenderAnalysis/DocxExtractionTask/taskDocxExtraction_api_fa';
// import { StreamingTaskApi } from '@/api/projects_api/taskOutlineAnalysis_api';
import type { Type_DocxExtractionTaskDetail, Type_DocxExtractionTaskUpdate } from '@/components/projects/TenderAnalysis/DocxExtractionTask/taskDocxExtraction_api_fa';
import type {
  TaskStatus,
  TaskLockStatus,
...
export const useDocxExtraction = () => {
  ...
  const DocxExtractionQuery = (
    ...
    const query = useQuery({
      queryKey: ['docxExtractionTask', projectId],
      queryFn: async () => {
        const result = await DocxExtractionFastApi.getDocxExtractionTask(projectId);
  ...
  const updateDocxExtractionTask = useMutation({
    ...
    // 构建要上传的数据，格式为Type_DocxExtractionTaskUpdate
      ...
      const result = await DocxExtractionFastApi.updateDocxExtractionTask(
        projectId, 
        taskData as Type_DocxExtractionTaskUpdate);
      console.log('✅ 成功更新文档提取的任务数据:', result);
      return { result, projectId, status };
    },
    onSuccess: (data, variables) => {
      // Invalidate the query to trigger a refetch
      queryClient.invalidateQueries({
        queryKey: ['docxExtractionTask', variables.projectId]
      });
      ...
      if (data.status === 'COMPLETED' || variables.status === 'COMPLETED') {
        ...
        queryClient.invalidateQueries({
          queryKey: ['projectStage', variables.projectId]
        });
      }
    }
  });
