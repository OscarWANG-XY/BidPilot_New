 ...
 // const { initGphCaptcha, showGphCaptcha } = useGphCaptcha(handleSendSMSCode);
  ...
  const validateForm = () => {
    ...
    if (loginType === 'code') {
      ...
      if (!isValidPhone(formState.phone)) {
        toast({ variant: "destructive", title: "错误", description: "请输入有效的手机号" });
        return false;
      }
    }
    ...
    if (loginType === 'password') {
      if (!formState.phoneOrEmail) {
        toast({ variant: "destructive", title: "错误", description: "请输入手机号或邮箱" });
        return false;
      }
      
      // 判断输入是手机号还是邮箱，并进行相应验证
      const isPhone = /^\d+$/.test(formState.phoneOrEmail); // 如果全是数字，假定是手机号
      
      if (isPhone) {
        if (!isValidPhone(formState.phoneOrEmail)) {
          toast({ variant: "destructive", title: "错误", description: "请输入有效的手机号" });
          return false;
        }
      } else {
        if (!isValidEmail(formState.phoneOrEmail)) {
          toast({ variant: "destructive", title: "错误", description: "请输入有效的邮箱地址" });
          return false;
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[LoginForm] handleSubmit 登录类型：', loginType);
      console.log('[LoginForm] handleSubmit 表单数据：', formState);
      ...
      if (loginType === 'code') { // 如果是验证码登录
        ...
        await login(LoginMethod.PASSWORD, {
          phoneOrEmail: formState.phoneOrEmail,
            ...
            <div className="grid gap-6">
              ...
              {loginType === 'code' ? (
                ...
                <PasswordLoginForm
                  ...
                  password={formState.password}
                  isLoading={isLoading}
                  onPhoneOrEmailChange={(value) => handleChange('phoneOrEmail', value)}
