...
import { PhaseDetailView } from '../../playground/ProjectManagement2/_07.b_PhaseDetailView'
...
import { StageType, StageStatus, ProjectStatus, ProjectStage, TaskStatus } from '@/types/projects_dt_stru'
//import { TaskStatus } from '@/types/projects_stages_dt_stru'
...
export const ProjectPhaseView: React.FC<ProjectPhaseViewProps> = ({ 
  ...
  //console.log('ProjectPhaseView组件渲染')
  //console.log('phases数据：', phases)

  // 获取当前阶段数据 - 确保类型匹配
  const currentPhase = phases.find(phase => String(phase.id) === phaseId)
  //console.log('currentPhase数据：', currentPhase)
  ...
  // 确定阶段状态， 未来被引用比较，来判断用户正在查看的阶段是项目当前进度之前，还是未来的阶段
  const currentStageIndex = Object.values(StageType).indexOf(currentProjectStage);
  const phaseStageIndex = Object.values(StageType).indexOf(currentPhase.stage);
  
  // 使用PhaseStatus来确定阶段状态 - 优先使用传入的phaseStatus，否则使用阶段自身的status
  const currentPhaseStatus = phaseStatus || currentPhase.status;
  const isCompleted = currentPhaseStatus === StageStatus.COMPLETED;
  const isInProgress = currentPhaseStatus === StageStatus.IN_PROGRESS;
  const isBlocked = currentPhaseStatus === StageStatus.BLOCKED;
  const isNotStarted = currentPhaseStatus === StageStatus.NOT_STARTED;
