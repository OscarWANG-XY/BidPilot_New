...
class AgentStateManager:
    ...
    async def init_or_restore_agent(self) -> Tuple[AgentState, StageEnum, str]:
        ...
        if not current_state:
            print("当前状态为空，准备初始化")
            logger.info(f"AGENT 尚未启用，将进行初始化")
        ...
        # 如果存在当前状态，执行恢复
            ...
            if active_stage_config["stage_type"] == "END_STAGE" and current_state.stage_status == StageStatus.COMPLETED:
                logger.info(f"AGENT 已经成功完成所有阶段任务，请查看结果")
                return None

            else:
            # 其他所有情况：
                logger.info(f"AGENT 已启用，将恢复至最新状态")
    ...
    def _determine_stage(self, initial_state: AgentState) -> None:
        # 决定接下去的执行步骤
        if initial_state == None:
            raise ValueError("AGENT未初始化或恢复状态，无法确定下个步骤")
        # 除非stage_status是完成，执行下一个stage，否则都执行当前stage
        if initial_state.stage_status == StageStatus.COMPLETED:
            current_stage = initial_state.active_stage
            next_stage = stage_config[current_stage]["next_stage"]
            if not next_stage:
                raise ValueError("无下个阶段，AGENT可能已经完成所有任务")
